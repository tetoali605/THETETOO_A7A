...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '🎀🎖تـم تفعيل✔️ بـوت سـورس تيـتو هنـا 😻 قـناة السـورس @no_no2\n🎀🎖Order by > @'..msg.from.username
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '🎀🎖تـم تــعطيـل✔️ بـوت سـورس تيـتو هنـا 😿 قـناة السـورس @no_no2\n🎀🎖 Order by > @'..msg.from.username
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "🎀🎖فقــط للادمنـيةة انتـة دايـح مـو ادمــن"
  end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'No rules📋 available🎀🎖 /n🎀🎖 Order by > @'..msg.from.username
  end
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return "🎀🎖فقــط للادمنــيـةة انتــةة دايــح مــو ادمــن"
  end
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['mute_group'] then
			data[tostring(target)]['settings']['mute_group'] = 'no'
		end
	end	
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'addbot' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '🎀🎖عــزيزي البـوت مفعـل 😻 و لا تـظل تلـح 😽 بـووت ســورس تيـتو هـنا @no_no2', ok_cb, false)
			end
		...
		if matches[1] == 'rembot' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '🎀🎖عــزيـزي الـبوت معـطل 😼 و لا تـظل تلـح 😹 بـووت سـورس تيـتو هنا @no_no2', ok_cb, false)
			end
	...
	if matches[1] == 'kickme' then
		...
		if matches[1] == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '*Error Link⚠')
		...
		if matches[1] == "setabout" and is_momod(msg) then
			...
			return "Supergroup Description📃 set✔ \n🎀🎖 Order by > @"..msg.from.username
		end
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					data[tostring(msg.to.id)]['settings']['mute_group'] = 'yes'
                    save_data(_config.moderation.data, data)
		...
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					data[tostring(msg.to.id)]['settings']['mute_group'] = 'no'
                    save_data(_config.moderation.data, data)
		...
		if matches[1] == "stop" or matches[1] == "stop" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "stop" or matches[1] == "stop" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "stop" or matches[1] == "stop" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'helppp' and not is_owner(msg) then
			text = "welcome "..msg.from.username.."\n🎀🎖 used help is /help"
			reply_msg(msg.id, text, ok_cb, false)
		elseif matches[1] == 'helppp' and is_owner(msg) then
