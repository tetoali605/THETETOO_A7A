...
local function run(msg, matches)
  if matches[1]:lower() == 'vip_team1' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /vip_team1 ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "vip_team1" then -- Put everything you like :)
