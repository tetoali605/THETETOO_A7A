...
local function create_group(msg)
    ...
    if is_sudo(msg) or is_realm(msg) and is_admin1(msg) then
		...
		return 'تم صنع [ '..string.gsub(group_name, '_', ' ')..' ]المجموعة بنجاح 📛🙂'
	end
end
...
local function res_user_support(cb_extra, success, result)
   ...
   local support_id = result.peer_id
	if get_cmd == 'addsupport' then
		...
		send_large_msg(receiver, " ["..support_id.."] عزيزي تم رفعه 👷🏻 مدير على كل مجموعات البوت 🙂🚀")
	elseif get_cmd == 'removesupport' then
		support_remove(support_id)
		send_large_msg(receiver, " ["..support_id.."]عزيزي تم ازالته 📛 من ادارة المجموعات جميعها ⚠️😐")
	end
end
...
function run(msg, matches)
    ...
    if matches[1] == 'اصنع مجموعة' and matches[2] then
    ...
    local receiver = get_receiver(msg)
		...
		if matches[1] == 'ارفع مساعد' and matches[2] then
			if string.match(matches[2], '^%d+$') then
