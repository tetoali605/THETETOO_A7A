...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
       local text = 'مرحــباً حظرت المطور المجموعه قمت بإزالته🙂 اسم المجموعة :🔹'..msg.to.title
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function vip_team1(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		  ...
		  local iDev1 = "مرحــباً حظرت المطور المجموعه مفعــله في هذه المجموعه🙂🚀"
...
local text = "💎- ٱلَٱسًےـمِےـ: " ..string.gsub(msg.from.print_name, "_", " ").. "\n💯- ٱلَمِےـعَےـرفُےـ: @"..(msg.from.username or '----').."\n⁉️- ايــديك: "..msg.from.id.."\n✳️- ٱڛمُ ٱڷڲږوٌٻ: " ..string.gsub(msg.to.print_name, "_", " ").. "\n🚀- ٱڷڲږوٌٻ: "..msg.to.id        return reply_msg(msg.id, text, ok_cb, false)
		...
		if matches[1] == "وضع اسم للمجموعة" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "وضع وصف للمجموعة" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == 'وضع قوانين للمجموعة' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if matches[1] == 'وضع صورة للمجموعة' and is_momod(msg) then
		...
		if matches[1] == 'امنع' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1] == 'اسمح' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1] == 'وضع عدد التكرار' then
			if not is_momod(msg) then
				return
			end
...
return 'الفيديوهات قمت بفتحها الان 🔓\n🌝 الامر بواسطة :️ @'..msg.from.username 
		...
		if matches[1] == "اسكت" or matches[1] == "اسكت" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "اسكت" or matches[1] == "اسكت" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
				...
				elseif is_momod(msg) then
					...
					return "["..user_id.."] تم اسكاتك ☠❌ بنجاح 🔨⚔"
				end
			elseif matches[1] == "اسكت" or matches[1] == "اسكت" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'الاوامر' and not is_owner(msg) then
