...
local function run(msg, matches)
 ...
 local text = '🚀- اڛمُ ٱڷٱوٌڷ  : '..(msg.from.first_name or 'لا يوجد ')..'\n'
  ...
  if hash then
    ...
    if msg.from.id == tonumber(Arian) then
     text = text..'✔️- مُوٌڦعُڲ : —انت مطور— \n'
    elseif is_sudo(msg) then
     text = text..'💡- مُوٌڦعُڲ : —انت مطور— \n'
    elseif is_owner(msg) then
     text = text..'💯- مُوٌڦعُڲ : —انت مشرف— \n'
    elseif is_momod(msg) then
     text = text..'🚀- مُوٌڦعُڲ : —انت ادمن— \n'
    else
     text = text..'🐸- مُوٌڦعُڲ : —انت عضو--\n'
    end
    else
     text = text..'💡 whois : '..value..'\n'
    end
   ...
   user_info_msgs = tonumber(redis:get(um_hash) or 0)
   text = text..'🌝- ٱڷږڛٱئڷ ٱڷمُږڛڷةّ :'..user_info_msgs..'\n\n'
   if msg.to.type == 'chat' then
